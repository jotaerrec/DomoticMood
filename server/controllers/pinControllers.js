...
module.exports = {
  create: async function (req, res) {
    ...
    try {
      ...
      if (verifyPin)
      return res.status(200).json(`Este pin ya esta en uso`);
  ...
  updatePin: async function (req, res) {
    ...
    try {
      ...
      res.status(200).json({message: "Error al actualizar el pin. Intentelo nuevamente"})
    }
  },
  delete: async function (req, res) {
    ...
    try {
      if (!pin)
        return res.status(204).json({ error: "Se requiere rellenar los campos" });
  ...
  getAll: async function (req, res) {
    try {
      ...
      const user = await User.findById(userID);
      if (!user ) 
        return res.status(200).json("Este usuario no existe");
